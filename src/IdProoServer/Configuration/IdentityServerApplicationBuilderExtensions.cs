...
public static class IdentityServerApplicationBuilderExtensions
{
    ...
    public static void Validate(this IApplicationBuilder app)
    {
        ...
        var logger = loggerFactory.CreateLogger("IdProoServer.Startup");
        logger.LogInformation("Starting IdProoServer version {version}", typeof(IdProoServer.Hosting.IdentityServerMiddleware).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>().InformationalVersion);
